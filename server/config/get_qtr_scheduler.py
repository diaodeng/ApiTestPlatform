...
# 重写Cron定时
class QtrCronTrigger(CronTrigger):
    @classmethod
...
qtr_scheduler = BackgroundScheduler()
qtr_scheduler.configure(jobstores=job_stores, executors=executors, job_defaults=job_defaults)


class QtrSchedulerUtil:
    """
    定时任务相关方法
    """

    @classmethod
    async def init_qtr_scheduler(cls, query_db: Session = SessionLocal()):
        """
        应用启动时初始化定时任务
        :return:
        """
        ...
        query_db.close()
        qtr_scheduler.add_listener(cls.qtr_scheduler_event_listener, EVENT_ALL)
        logger.info("QTR初始定时任务加载成功")

    @classmethod
    async def close_qtr_scheduler(cls):
        """
        应用关闭时关闭定时任务
        :return:
        """
        qtr_scheduler.shutdown()
        logger.info("关闭QTR定时任务成功")
    ...
    def get_scheduler_job(cls, job_id):
        ...
        :return: 任务对象
        """
        query_job = qtr_scheduler.get_job(job_id=str(job_id))
    ...
    def add_scheduler_job(cls, job_info):
        ...
        :return:
        """
        qtr_scheduler.add_job(
            func=eval(job_info.invoke_target),
            trigger=QtrCronTrigger.from_crontab(job_info.cron_expression),
    ...
    def remove_scheduler_job(cls, job_id):
        ...
        :return:
        """
        qtr_scheduler.remove_job(job_id=str(job_id))

    @classmethod
    def qtr_scheduler_event_listener(cls, event):
        # 获取事件类型和任务ID
