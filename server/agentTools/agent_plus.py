...
# 配置loguru，将日志保存到当前目录下的app.log文件
logger.add(log_path, rotation=timedelta(days=1), level="INFO")  # rotation是可选的，用于设置日志文件的轮换策略
...
def check_and_create_config():
    ...
    if not os.path.exists(config_file):
        ...
        # 设置是否自启动服务的默认值为空
        config['Settings']['start_flag'] = 'False'
...
def main():
    ...
    # 至少传入app
    op = OperateWinProcess((app, am))
    # 读取配置文件
    config = configparser.ConfigParser()
    config.read('config.ini')
    config_setting = config['Settings']
    if config_setting.get('start_flag') == 'True':
        op.start_service()
