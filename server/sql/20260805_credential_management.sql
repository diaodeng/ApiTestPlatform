...
CREATE TABLE IF NOT EXISTS auth_credential_auth_config (
  auth_config_id BIGINT NOT NULL AUTO_INCREMENT COMMENT '认证配置主键', credential_id BIGINT NOT NULL COMMENT '凭证主键',
...
CREATE TABLE IF NOT EXISTS auth_credential_binding (
  binding_id BIGINT NOT NULL AUTO_INCREMENT COMMENT '绑定主键', binding_name VARCHAR(128) NOT NULL COMMENT '绑定名称', credential_id BIGINT NOT NULL COMMENT '凭证主键',
  business_type VARCHAR(32) NOT NULL COMMENT '业务类型：web_case/ticket_log_pull/ticket_remote_sync', projection_type VARCHAR(32) NOT NULL COMMENT '投影类型：playwright_storage/http_cookie/http_header',
  target_url VARCHAR(1000) NOT NULL DEFAULT '', target_host_patterns JSON NULL, sharing_mode VARCHAR(32) NOT NULL DEFAULT 'shared_read', writeback_enabled TINYINT(1) NOT NULL DEFAULT 0, enabled TINYINT(1) NOT NULL DEFAULT 1,
  create_by VARCHAR(64) NOT NULL DEFAULT '', create_time DATETIME NOT NULL, update_by VARCHAR(64) NOT NULL DEFAULT '', update_time DATETIME NOT NULL, remark VARCHAR(500) NULL, del_flag CHAR(1) NOT NULL DEFAULT '0',
