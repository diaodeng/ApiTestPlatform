...
class SysJob(Base):
    ...
    job_kwargs = Column(long_text_type('utf8_general_ci'), nullable=True, comment='关键字参数')
    ...
    run_status = Column(Integer, nullable=True, default=6, comment='任务执行状态，TaskStatusEnum')
...
class SysJobLog(Base):
    ...
    exception_info = Column(long_text_type('utf8_general_ci'), nullable=True, default='', comment='异常信息')
    create_time = Column(DateTime, nullable=True, default=datetime.now, comment='创建时间')
