...
from module_admin.service.user_service import CrudUserRoleModel, UserService
...
async def get_system_role_dept_tree(request: Request,
                                    role_id: int,
...
async def add_system_role(request: Request,
...
async def edit_system_role(request: Request,
...
async def edit_system_role_datascope(request: Request,
...
async def delete_system_role(request: Request,
                             role_ids: str,
                             query_db: Session = Depends(get_db),
                             current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    try:
...
@log_decorator(title='角色管理', business_type=2)
async def reset_system_role_status(request: Request,
                                   edit_role: AddRoleModel,
                                   query_db: Session = Depends(get_db),
                                   current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    try:
...
async def get_system_allocated_user_list(request: Request,
                                         ...
                                         query_db: Session = Depends(get_db)):
    try:
        role_user_allocated_page_query_result = RoleService.get_role_user_allocated_list_services(query_db,
                                                                                                  user_role,
                                                                                                  is_page=True)
        logger.info('获取成功')
...
@log_decorator(title='角色管理', business_type=4)
async def add_system_role_user(request: Request,
...
@log_decorator(title='角色管理', business_type=4)
async def cancel_system_role_user(request: Request,
