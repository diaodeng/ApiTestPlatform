...
class AiProviderProtocolService:
    ...
    def _mask_headers(cls, headers):
        ...
        for key, value in headers.items():
            if key.lower() in sensitive_keys:
                if len(value) > _API_KEY_MASK_MIN_LEN:
    ...
    @classmethod
    def _truncate_text(cls, text: str, max_len: int = 100) -> str:
        if len(text) <= max_len:
            return text
        return text[:max_len] + "..." + str(len(text)) + "chars"

    @classmethod
    def _format_request_body(cls, **kwargs):
        for key in ("json", "data", "content", "params"):
            if key in kwargs:
                body = kwargs[key]
                if body is None:
                    return "-"
                try:
                    ...
                    body_str = str(body)
                return cls._truncate_text(body_str)
        return "-"
    ...
    def _request(
        ...
        **kwargs: Any,
        ...
        :return: 成功响应
        """
        masked_headers = cls._mask_headers(headers)
        body_summary = cls._format_request_body(**kwargs)
        timeout = timeout_sec or cls.DEFAULT_TIMEOUT_SEC
        logger.info(
            "AI model request: method=" + str(method) + " url=" + str(url) + " timeout=" + str(timeout) + "s"
            + " headers=" + str(masked_headers) + " body=" + body_summary
        )
        with httpx.Client(timeout=timeout, follow_redirects=False) as client:
            try:
                ...
                logger.info(
                    "AI model success: method=" + str(method) + " url=" + str(url)
                    + " status=" + str(response.status_code) + " size=" + str(len(response.content)) + "bytes"
                )
                return response
            except httpx.HTTPStatusError as exc:
                logger.error(
                    "AI model HTTP error: method=" + str(method) + " url=" + str(url)
                    + " status=" + str(exc.response.status_code)
                    + " body=" + cls._truncate_text(exc.response.text[:200])
                )
                raise
            except httpx.TimeoutException as exc:
                logger.error(
                    "AI model timeout: method=" + str(method) + " url=" + str(url)
                    + " timeout=" + str(timeout) + "s reason=" + str(exc)
                )
                raise
            except Exception as exc:
                logger.error(
                    "AI model exception: method=" + str(method) + " url=" + str(url)
