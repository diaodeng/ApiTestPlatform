...
class CommonService:
    ...
    def upload_service(cls, request: Request, file: UploadFile):
        ...
        if not UploadUtil.check_file_extension(file):
            result = {'is_success': False, 'message': '文件类型不合法'}
        else:
    ...
    def download_services(cls, background_tasks: BackgroundTasks, file_name, delete: bool):
        ...
        if '..' in file_name:
            result = {'is_success': False, 'message': '文件名称不合法'}
        elif not UploadUtil.check_file_exists(filepath):
            result = {'is_success': False, 'message': '文件不存在'}
        else:
            result = {'is_success': True, 'result': UploadUtil.generate_file(filepath), 'message': '下载成功'}
            if delete:
