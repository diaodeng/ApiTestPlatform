...
class QtrSuite(Base, BaseModel):
    ...
    suite_id: Mapped[int] = mapped_column(BigInteger, unique=True, primary_key=True, nullable=False, default=snowIdWorker.get_id,
                             comment='测试套件Id')
    ...
    status: Mapped[int] = mapped_column(Integer, nullable=True, default=2, comment='环境状态（2正常 1停用）')  # QtrDataStatusEnum
...
class QtrSuiteDetail(Base, BaseModel):
    ...
    suite_detail_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, nullable=False, default=snowIdWorker.get_id,
                                    comment='测试套件详情Id')
    suite_id: Mapped[int] = mapped_column(BigInteger, nullable=False, comment='测试套件Id')
    data_id: Mapped[int] = mapped_column(BigInteger, nullable=False, comment='数据Id')
