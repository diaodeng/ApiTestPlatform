...
class DebugTalkDao:
    ...
    def get_debugtalk_by_id(cls, db: Session, debugtalk_id: int):
        ...
        debugtalk_info = (
            ...
            .filter(HrmDebugTalk.debugtalk_id == debugtalk_id, HrmDebugTalk.status == 0, HrmDebugTalk.del_flag == 0)
            .first()
        )
    ...
    @classmethod
    def get_debugtalk_list(cls, db: Session, page_object: DebugTalkQueryModel, data_scope_sql: DataScopeExpr):
        """
        ...
        """
        debugtalk_list = (
            db.query(HrmDebugTalk, HrmProject.project_name)
            .outerjoin(HrmProject, HrmDebugTalk.project_id == HrmProject.project_id)
            .filter(
                or_(
                    HrmDebugTalk.project_id == page_object.project_id if page_object.project_id else True,
                    HrmDebugTalk.project_id == None,
                )
            )
            .filter(
            ...
            .order_by(HrmDebugTalk.debugtalk_id)
            .order_by(HrmDebugTalk.create_time.desc(), HrmDebugTalk.update_time.desc())
        )

        debugtalk_list = PageUtil.paginate(
            debugtalk_list, page_object.page_num, page_object.page_size, page_object.is_page
        )
    ...
    def get_debugtalk_detail_by_id(cls, db: Session, id: int):
        ...
        debugtalk_info = (
            db.query(HrmDebugTalk)
            .filter((or_(HrmDebugTalk.debugtalk_id == id, HrmDebugTalk.project_id == id)), HrmDebugTalk.del_flag == 0)
            .first()
        )
    ...
    def edit_debugtalk_dao(cls, db: Session, debugtalk: dict):
        ...
        :return: 编辑校验结果
        """
        db.query(HrmDebugTalk).filter(HrmDebugTalk.debugtalk_id == debugtalk.get("debugtalk_id")).update(debugtalk)
    ...
    def delete_debugtalk_dao(cls, db: Session, debugtalk: DebugTalkModel):
        ...
        :return:
        """
        db.query(HrmDebugTalk).filter(HrmDebugTalk.project_id == debugtalk.project_id).update(
            {
                HrmDebugTalk.del_flag: "2",
                HrmDebugTalk.update_by: debugtalk.update_by,
