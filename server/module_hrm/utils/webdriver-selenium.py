...
class MyDriver():
    ...
    def __init__(self,
        ...
        try:
            ...
            # logger.error(e, exc_info=True)
            logger.info("=" * 60)
            logger.info(" Chrome 浏览器初始化失败。信息：")
            logger.info(mydriver_log)
            logger.info('您可以检查下：')
            logger.info("1. 是否存在./chrome/chromedriver.exe 或 PATH 中是否存在 chromedriver.exe")
            logger.info(
            ...
            logger.info("谷歌浏览器linux版本下载地址https://www.google.com/chrome/?platform=linux")
            logger.info("3. 如不是以上问题，请提issue，附上报错信息和您的环境信息")
            logger.info("=" * 60)
            # auto.prompt("按回车键继续......")
            raise e
    ...
    def wait_find_elements(self, local, timeout=10, poll_frequency=0.5):
        return WebDriverWait(self.driver, timeout, poll_frequency).until(
            lambda driver: driver.find_elements(*self._locator(local)))

    def wait_find_element(self, local, timeout=10, poll_frequency=0.5) -> WebElement:
        return WebDriverWait(self.driver, timeout, poll_frequency).until(
            lambda driver: driver.find_element(*self._locator(local)))
...
if __name__ == "__main__":
    """
    C:\Program Files (x86)\Google\Chrome\Application\chrome.exe --remote-debugging-port=1888 --remote-debugging-address=10.12.196.160 --user-data-dir="C:\myself\chromedata"
    """
    # test_remote_driver()
    driver = MyDriver(
    ...
    driver.go_url("http://www.baidu.com")
    time.sleep(2)
