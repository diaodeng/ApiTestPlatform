...
class TicketAiResultSchemaUtil:
    """工单 AI 分析结果清洗工具 — 供服务端与 Agent 客户端共用的无副作用纯函数集合。
    ...
    def sanitize_result_payload(
        ...
        :return: (清洗后的结果, 清洗动作描述列表，如 ["$.ticket_no: 已剔除额外字段"])
        """
        ...
        if evidence_items_schema.get("type") == "string" and isinstance(cleaned.get("evidence"), list):
            ...
            for index, item in enumerate(cleaned["evidence"]):
                if isinstance(item, str):
                    ...
                    continue
                normalized_evidence.append(TicketAiResultSchemaUtil._coerce_evidence_entry(item))
                actions.append(
