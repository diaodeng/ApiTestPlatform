...
(() => {
  ...
  document.addEventListener("click", (event) => {
    ...
    emit({ stepName: text ? `点击 ${text}` : "点击元素", actionType: "click", params: {}, assertions, rawEvent: { eventType: "click", tagName: target.tagName }, targetSnapshot: buildSnapshot(target) });
  }, true);
  document.addEventListener("change", (event) => {
    ...
    if (tag === "input") {
      ...
      if (["checkbox", "radio"].includes(type)) {
        emit({ stepName: target.checked ? "勾选元素" : "取消勾选元素", actionType: target.checked ? "check" : "uncheck", params: {}, assertions: [], rawEvent: { eventType: "change", tagName: target.tagName }, targetSnapshot: buildSnapshot(target) });
        return;
      }
      emit({ stepName: "填写输入框", actionType: "fill", params: { value: target.value || "" }, assertions: [], rawEvent: { eventType: "change", tagName: target.tagName }, targetSnapshot: buildSnapshot(target) });
      return;
    }
    if (tag === "textarea") {
      emit({ stepName: "填写文本域", actionType: "fill", params: { value: target.value || "" }, assertions: [], rawEvent: { eventType: "change", tagName: target.tagName }, targetSnapshot: buildSnapshot(target) });
      return;
    }
    if (tag === "select") {
      const values = Array.from(target.selectedOptions || []).map((item) => item.value || item.label || "");
      emit({ stepName: "选择下拉项", actionType: "select_option", params: { values }, assertions: [], rawEvent: { eventType: "change", tagName: target.tagName }, targetSnapshot: buildSnapshot(target) });
    }
  }, true);
  document.addEventListener("keydown", (event) => {
    ...
    const target = event.target instanceof Element ? event.target : null;
    if (!target) return;
    emit({ stepName: "键盘按下 Enter", actionType: "press", params: { key: "Enter" }, assertions: [], rawEvent: { eventType: "keydown", key: "Enter" }, targetSnapshot: buildSnapshot(target) });
  }, true);
})();
"""
...
class RecorderSession:
    ...
    async def emit(self, payload: dict[str, Any], event_type: str = "record_event") -> None:
        ...
        await self.sender({"type": event_type, "recording_id": self.recording_id, "event_index": self.event_index, "payload": payload})
...
class WebTestService:
    ...
    async def handle_request(cls, req_data: dict[str, Any], event_sender: EventSender | None = None) -> dict[str, Any]:
        ...
        return {"request_type": 3, "command": command, "success": False, "status": "failed", "message": f"unsupported webui command: {command}"}

    @classmethod
    async def _start_recording(cls, req_data: dict[str, Any], event_sender: EventSender | None) -> dict[str, Any]:
        if async_playwright is None:
            return {"request_type": 3, "command": "start_recording", "success": False, "status": "failed", "message": "playwright 未安装，无法录制"}
        if event_sender is None:
            return {"request_type": 3, "command": "start_recording", "success": False, "status": "failed", "message": "缺少事件发送器，无法实时推送录制结果"}
        recording_id = int(req_data.get("recordingId") or 0)
        if not recording_id:
            return {"request_type": 3, "command": "start_recording", "success": False, "status": "failed", "message": "recordingId 不能为空"}
        async with cls._lock:
            if recording_id in cls._recorders:
                return {"request_type": 3, "command": "start_recording", "success": True, "status": "accepted", "recording_id": recording_id, "message": "录制已在进行中"}
            session = RecorderSession(recording_id=recording_id, agent_code=str(req_data.get("agentCode") or ""), sender=event_sender, browser_name=str(req_data.get("browserName") or "chromium"), headless=bool(req_data.get("headless", False)), start_url=str(req_data.get("startUrl") or ""), options=_as_dict(req_data.get("recordingOptions")))
        ...
        try:
            ...
            session.page.on("framenavigated", lambda frame: asyncio.create_task(cls._handle_navigation(frame, session)))
            await session.page.goto(session.start_url)
            await session.emit({"stepName": "打开页面", "actionType": "goto", "params": {"url": session.start_url}, "assertions": [], "rawEvent": {"eventType": "goto"}, "targetSnapshot": None})
            return {"request_type": 3, "command": "start_recording", "success": True, "status": "accepted", "recording_id": recording_id}
        except Exception as exc:
            logger.exception(exc)
            async with cls._lock:
                cls._recorders.pop(recording_id, None)
            try:
                await session.sender({"type": "record_error", "recording_id": recording_id, "message": str(exc), "payload": {"message": str(exc)}})
                await session.close()
            except Exception:
                pass
            return {"request_type": 3, "command": "start_recording", "success": False, "status": "failed", "message": str(exc)}
    ...
    async def _handle_navigation(cls, frame: Any, session: RecorderSession) -> None:
        if session.page is None or frame != session.page.main_frame:
            return
        await session.emit({"stepName": "页面跳转", "actionType": "goto", "params": {"url": frame.url}, "assertions": [], "rawEvent": {"eventType": "framenavigated"}, "targetSnapshot": None}, event_type="record_status")
    ...
    async def _stop_recording(cls, req_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if session is None:
            return {"request_type": 3, "command": "stop_recording", "success": False, "status": "failed", "message": "录制会话不存在"}
        await session.sender({"type": "record_finished", "recording_id": recording_id, "payload": {"eventCount": len(session.events), "lastUrl": session.page.url if session.page else ""}})
        await session.close()
        return {"request_type": 3, "command": "stop_recording", "success": True, "status": "stopped", "recording_id": recording_id, "data": {"eventCount": len(session.events)}}

    @classmethod
    async def _run_case(cls, req_data: dict[str, Any]) -> dict[str, Any]:
        if async_playwright is None:
            return {"request_type": 3, "command": "run_case", "success": False, "status": "failed", "message": "playwright 未安装，无法执行 Web 用例"}
        ...
        try:
            ...
            for raw_step in steps:
                step_result = await cls._run_single_step(page, _as_dict(raw_step))
                result_steps.append(step_result)
                if step_result["status"] != "passed":
                    overall_success = False
                    if not bool(runtime_options.get("continueOnFailure", False)):
                        break
            return {"request_type": 3, "command": "run_case", "success": overall_success, "status": "success" if overall_success else "failed", "result": {"steps": result_steps, "pageUrl": page.url if page else start_url}}
        except Exception as exc:
            logger.exception(exc)
            return {"request_type": 3, "command": "run_case", "success": False, "status": "failed", "message": str(exc), "result": {"steps": result_steps, "pageUrl": page.url if page else start_url}}
        finally:
            if page is not None:
                await page.close()
    ...
    async def _run_single_step(cls, page: Any, step: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"stepId": step_id, "stepName": step_name, "status": "passed", "durationMs": int((time.perf_counter() - started_at) * 1000), "attempts": attempts, "pageUrl": page.url}
        except Exception as exc:
            logger.exception(exc)
            return {"stepId": step_id, "stepName": step_name, "status": "failed", "durationMs": int((time.perf_counter() - started_at) * 1000), "attempts": attempts, "pageUrl": page.url, "error": str(exc)}
