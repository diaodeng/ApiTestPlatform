<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      ...
      <el-form-item :label="dataName+'ID'" prop="ruleId">
        <el-input
      ...
      <el-form-item :label="dataName+'名称'" prop="name">
        <el-input
  ...
  listMockRule(queryParams.value).then(response => {
    ...
    nextTick(() => {
      ...
      if (allSelectIds.value && allSelectIds.value.length > 0) {
        if (tableRef.value && ruleList.value) {
          ruleList.value.forEach(item => {
            if (allSelectIds.value.some(dataId => dataId === item.ruleId)) {
...
function handleSelect(selection, row) {
  let pageIds = ruleList.value.map(item => item.ruleId);
  let pageSelectIds = selection.map(item => item.ruleId);
  let delIds = pageIds.filter(item=> !pageSelectIds.includes(item));
  pageSelectIds.forEach((dataId) => {
    if (!allSelectIds.value.some((ruleId) => ruleId === dataId)) {
...
function handleSelectionChange(selection) {
  ...
  // let pageIds = ruleList.value.map(item => item.ruleId);
  // 从选中数据中排除当前页的数据
  // allSelectIds.value = allSelectIds.value.filter((dataId) => !ruleList.value.some(item=>item.ruleId === dataId));

  // 添加当前页选中的数据
  // selection.forEach((item) =>{
  //   if (!allSelectIds.value.some((ruleId)=>ruleId === item.ruleId)){
  //     allSelectIds.value.push(item.ruleId);
  //   }
  // });
...
function handleSelectAll(selection) {
  ...
  if (pageSelectIds && pageSelectIds.length > 0) {
    pageSelectIds.forEach((item) => {
      if (!allSelectIds.value.some((ruleId) => ruleId === item)) {
