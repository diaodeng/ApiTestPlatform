      ...
      >
        <template #default="scope">
          <el-button
            ...
            type="warning"
            icon="Histogram"
            @click="openErrorSummaryDialog(scope.row)"
            v-hasPermi="['hrm:report:detail']"
            title="异常统计"
          >
          </el-button>
          <el-button
      ...
      <el-container style="height: 100%">
        ...
        <el-main style="max-height: calc(100vh - 95px)">
          <RunDetail
            ...
            :view-type="runDetailViewTypeEnum.report"
          ></RunDetail>
        </el-main>
      </el-container>
    </el-dialog>
    <ErrorSummaryDialog
      v-model:open-error-summary-dialog="errorSummaryDialogIsOpen"
      :report-id="currentReportId"
    ></ErrorSummaryDialog>
  </div>
</template>
...
<script setup name="RunDetail">
  ...
  const errorSummaryDialogIsOpen = ref(false);
  ...
  function openErrorSummaryDialog(row) {
    currentReportId.value = row.reportId;
    errorSummaryDialogIsOpen.value = true;
  }
