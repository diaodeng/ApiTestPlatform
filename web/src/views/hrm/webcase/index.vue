...
<!--                            >-->
            ...
            <el-tab-pane label="执行记录" name="run">
                <el-form :inline="true" :model="runQueryParams" class="mb8">
                    ...
                    <el-form-item label="Agent编码">
                        <el-input
                            v-model="runQueryParams.agentCode"
                            placeholder="支持精确筛选"
                            clearable
                        />
                    </el-form-item>
                ...
                >
                    ...
                    <el-table-column label="状态" width="100">
                        <template #default="scope">
                            <el-tag
                                :type="getRunStatusMeta(scope.row.status).type"
                                >{{
                    ...
                    <el-table-column label="操作" width="180" fixed="right">
                        <template #default="scope">
                            ...
                            <el-button
                                ...
                                type="danger"
                                icon="Delete"
                                @click="handleDeleteRunRecords(scope.row)"
                                v-hasPermi="['hrm:webCase:remove']"
                                >删除</el-button
                            >
                        </template>
            ...
            <el-tab-pane label="录制记录" name="recording">
                <el-form
                    :inline="true"
                    :model="recordingQueryParams"
                    class="mb8"
                >
                    <el-form-item label="录制名称">
                        <el-input
                            v-model="recordingQueryParams.sessionName"
                            placeholder="请输入录制名称"
                            clearable
                    ...
                    <el-form-item label="录制状态">
                        <el-select
                            v-model="recordingQueryParams.status"
                            clearable
                            placeholder="全部状态"
                            style="width: 160px"
                        >
                ...
                >
                    ...
                    <el-table-column label="状态" width="110">
                        <template #default="scope">
                            <el-tag
                                :type="
                                    getRecordingStatusMeta(scope.row.status)
                                        .type
                                "
                                >{{
                                    getRecordingStatusMeta(scope.row.status)
                    ...
                    <el-table-column label="操作" width="330" fixed="right">
                        <template #default="scope">
                            ...
                            <el-button
                                ...
                                icon="CircleClose"
                                @click="
                                    handleCancelPreparedRecording(scope.row)
                                "
                                v-hasPermi="['hrm:webCase:record']"
                            >
                                取消准备
                            ...
                            <el-button
                                ...
                                type="danger"
                                icon="Delete"
                                @click="handleDeleteRecordingRecords(scope.row)"
                                v-hasPermi="['hrm:webCase:remove']"
                                >删除</el-button
                            >
                            <el-button
                                ...
                                icon="VideoPlay"
                                :disabled="
                                    !canUseRecordingResult(scope.row.status)
                                "
                    ...
                    <div class="step-table-toolbar">
                        ...
                        <div class="step-table-toolbar-actions">
                            ...
                            <el-button
                                ...
                                @click="
                                    openStepDetailByIndex(selectedStepIndex)
                                "
                                >编辑当前步骤</el-button
                            >
                        </div>
                ...
                <el-tab-pane label="高级 JSON" name="json">
                    <div class="json-toolbar">
                        <el-button
                            type="primary"
                            icon="Check"
                            @click="applyStepsTextToForm(true)"
                            >应用 JSON 到可视化</el-button
                        >
                        <el-button
                            icon="RefreshRight"
                            @click="syncStepsTextFromForm"
                            >使用当前可视化刷新 JSON</el-button
                        >
                    </div>
                    <AceEditor
                        v-model:content="stepsText"
                        lang="json"
                        :can-set="true"
                        height="560px"
                    />
        ...
        >
            <template v-if="currentStep">
                    ...
                    <el-row :gutter="16">
                        ...
                        <el-col :span="6">
                            <el-form-item label="动作类型">
                                <el-select
                                    ...
                                    style="width: 100%"
                                    @change="
                                        handleStepActionTypeChange(currentStep)
                                    "
                                >
                                    <el-option
                                        v-for="item in actionOptions"
                        ...
                        <el-col :span="4">
                            <el-form-item label="继续执行">
                                <el-switch
                                    v-model="currentStep.continueOnFailure"
                                />
                            </el-form-item>
                        </el-col>
                        <el-col :span="6">
                        ...
                        <el-col :span="6">
                            <el-form-item label="记录来源">
                                <el-input
                                    v-model="currentStep.recordOrigin"
                        ...
                        <el-col :span="6">
                            <el-form-item label="元素ID">
                                <el-input
                                    v-model="currentStep.elementId"
                                    placeholder="可选"
                                />
                            </el-form-item>
                        </el-col>
                        <el-col
                            v-if="currentStep.actionType === 'goto'"
                            :span="24"
                        >
                            <el-form-item label="跳转地址">
                                <el-input
                                    v-model="currentStep.params.url"
                                    placeholder="https://example.com/path"
                                />
                            </el-form-item>
                        </el-col>
                        ...
                        <el-col
                            v-else-if="currentStep.actionType === 'press'"
                            :span="24"
                        >
                        ...
                        <el-col
                            v-else-if="
                                currentStep.actionType === 'select_option'
                            "
                            :span="24"
                        >
                        ...
                        <el-col
                            v-else-if="
                                [
                                    'assert_page_contains',
                                    'assert_page_not_contains',
                                ].includes(currentStep.actionType)
                            "
                            :span="24"
                        >
                        ...
                        >
                            <el-form-item label="标题关键字">
                                <el-input
                                    v-model="currentStep.params.title"
                                    placeholder="请输入页面标题关键字"
                                />
                            </el-form-item>
                        </el-col>
                        <el-col
                            v-else-if="
                                currentStep.actionType === 'assert_url_contains'
                            "
                            :span="24"
                        >
                ...
                <el-card
                    v-if="stepNeedsTarget(currentStep.actionType)"
                    class="panel-card"
                    shadow="never"
                >
                    <template #header>
                    ...
                    <el-row :gutter="16" class="mb16">
                        <el-col :span="8">
                            <el-form-item label="元素文本" label-width="90px">
                                <el-input
                                    v-model="
                                        currentStep.targetSnapshot.elementText
                                    "
                                    placeholder="元素文本快照"
                                />
                            </el-form-item>
                        ...
                        <el-col :span="8">
                            <el-form-item label="指纹" label-width="90px">
                                <el-input
                                    v-model="
                                        currentStep.targetSnapshot.fingerprint
                                    "
                                    placeholder="元素指纹"
                                />
                            </el-form-item>
                        ...
                        <el-col :span="8">
                            <el-form-item label="稳定分" label-width="90px">
                                <el-input-number
                                    v-model="
                                        currentStep.targetSnapshot.stableScore
                                    "
                                    :min="0"
                                    :step="1"
                                    controls-position="right"
                        ...
                        <el-col :span="12">
                            <el-form-item label="页面URL" label-width="90px">
                                <el-input
                                    v-model="
                                        currentStep.targetSnapshot.context
                                            .pageUrl
                                    "
                                    placeholder="页面 URL"
                                />
                    ...
                    <div
                        v-if="currentStep.targetSnapshot?.locators?.length"
                        class="locator-list"
                    >
                        <div
                            v-for="(locator, locatorIndex) in currentStep
                                .targetSnapshot.locators"
                            :key="
                        ...
                        >
                            <div class="locator-header">
                                ...
                                <div class="locator-actions">
                                    ...
                                    <el-button
                                        ...
                                        :disabled="
                                            locatorIndex ===
                                            currentStep.targetSnapshot.locators
                                                .length -
                                                1
                                        "
                                        @click="
                            ...
                            <el-row :gutter="12">
                                <el-col :span="6">
                                    ...
                                    >
                                        <el-select
                                            ...
                                            style="width: 100%"
                                            @change="
                                                handleLocatorTypeChange(locator)
                                            "
                                        >
                                            <el-option
                                                v-for="item in locatorTypeOptions"
                                ...
                                >
                                    ...
                                    >
                                        <el-input
                                            v-model="locator.locatorValue.name"
                                            placeholder="按钮名称"
                                        />
                                    </el-form-item>
                                </el-col>
                                ...
                                >
                                    ...
                                    >
                                        <el-switch
                                            v-model="locator.locatorValue.exact"
                                        />
                                    </el-form-item>
                                </el-col>
                                <el-col
                                ...
                                >
                                    ...
                                    >
                                        <el-input
                                            v-model="locator.locatorValue.text"
                                            placeholder="定位文本"
                                        />
                                    </el-form-item>
                                </el-col>
                                ...
                                <el-col
                                    v-if="locator.locatorType === 'test_id'"
                                    :span="14"
                                >
                                    <el-form-item
                                        label="Test ID"
                                ...
                                <el-col
                                    v-if="locator.locatorType === 'id'"
                                    :span="14"
                                >
                                    <el-form-item label="ID" label-width="70px">
                                        <el-input
                                            v-model="locator.locatorValue.id"
                                            placeholder="元素 id"
                                        />
                                    </el-form-item>
                                </el-col>
                                <el-col
                                    v-if="locator.locatorType === 'name'"
                                    :span="14"
                                >
                                    <el-form-item
                                        label="Name"
                ...
                <el-card class="panel-card" shadow="never">
                    <template #header>
                        <div class="panel-header">
                            ...
                            <el-button
                                ...
                                plain
                                icon="Plus"
                                @click="addAssertion(currentStep)"
                                >新增断言</el-button
                            >
                        </div>
                    </template>
                    ...
                    <div v-if="currentStep.assertions?.length">
                            ...
                            <el-table-column label="断言类型" width="170">
                                <template #default="scope">
                                    <el-select
                                        ...
                                        style="width: 100%"
                                        @change="
                                            handleAssertionTypeChange(scope.row)
                                        "
                                    >
                                        <el-option
                        ...
                        <div class="step-detail-tip mt8">
                            展开每行可编辑该断言的完整定位器列表及优先级。
                        </div>
                    </div>
                    <el-empty
                        v-else
                        description="当前步骤暂无断言"
                        :image-size="70"
                    />
                </el-card>
            ...
            <template #footer>
                <el-button @click="showStepDetailDialog = false"
                    >关闭</el-button
                >
            </template>
        </el-dialog>
        ...
        >
            ...
            <template #footer>
                <el-button @click="showRecordingActionDialog = false"
                    >取消</el-button
                >
                <el-button
                    type="primary"
...
function normalizeStepParams(actionType, params) {
    ...
    if (["sleep", "wait"].includes(actionType)) {
        return {
            waitMs:
                Number.isFinite(waitMs) && waitMs >= 0 ? Math.round(waitMs) : 0,
            thinkTimeMs,
        };
    }
    if (
...
const projectNameMap = computed(() =>
    Object.fromEntries(
        projectOptions.value.map((item) => [item.projectId, item.projectName]),
    ),
);
const moduleNameMap = computed(() =>
    Object.fromEntries(
        moduleOptions.value.map((item) => [item.moduleId, item.moduleName]),
    ),
);
...
const runScopeProjectId = computed(
    () => runTargetCases.value?.[0]?.projectId || selectedCase.value?.projectId,
);
const runScopeModuleId = computed(
    () => runTargetCases.value?.[0]?.moduleId || selectedCase.value?.moduleId,
);
...
function getCaseName(webCaseId) {
    ...
    return (
        ...
        mergeCaseOptions(
            ...
            allCaseOptions.value,
            caseSelectOptions.value,
        ).find((item) => isSameId(item.webCaseId, targetId))?.caseName ||
        ""
    );
}
...
function isRuntimeProfileScopeMatch(profile, projectId, moduleId) {
    ...
    if (
        profileProjectId &&
        currentProjectId &&
        profileProjectId !== currentProjectId
    )
        return false;
    ...
    if (
        profileModuleId &&
        currentModuleId &&
        profileModuleId !== currentModuleId
    )
        return false;
...
function formatRuntimeProfileScope(profile) {
    const projectName = profile?.projectId
        ? getProjectName(profile.projectId) || profile.projectId
        : "全局";
    const moduleName = profile?.moduleId
...
function formatBrowserSessionLabel(session) {
    const name =
        `${session?.sessionName || session?.scopeKey || session?.sessionId || "未命名Session"}`.trim();
    const browserValue = `${session?.browserName || ""}`.trim().toLowerCase();
    const browserLabel = browserValue
        ? browserOptions.find((item) => item.value === browserValue)?.label ||
          browserValue
        : "";
...
function syncCaseOptions(...collections) {
    ...
    const mergedSelect = mergeCaseOptions(
        caseSelectOptions.value,
        ...collections,
    );
...
function getList() {
    ...
    listWebCase(queryParams.value)
        .then((response) => {
            pageDataList.value = (response.rows || []).map(
                (item) => normalizeCaseOption(item) || item,
            );
...
function getRunList() {
    ...
    return listWebRun(runQueryParams.value)
        .then((response) => {
...
function getRecordingList() {
    ...
    return listWebRecording(recordingQueryParams.value)
        .then((response) => {
...
function clearInvalidRuntimeProfileBindings() {
    ...
    if (
        runForm.value.runtimeProfileId &&
        !runtimeIds.has(normalizeIdValue(runForm.value.runtimeProfileId))
    ) {
        runForm.value.runtimeProfileId = undefined;
        if (runForm.value.stateSourceType === "cookie") {
            runForm.value.stateSourceType = "none";
        }
    }
    if (
        recordingForm.value.runtimeProfileId &&
        !runtimeIds.has(normalizeIdValue(recordingForm.value.runtimeProfileId))
    ) {
        recordingForm.value.runtimeProfileId = undefined;
        if (recordingForm.value.stateSourceType === "cookie") {
    ...
    if (
        runForm.value.browserSessionId &&
        !sessionIds.has(normalizeIdValue(runForm.value.browserSessionId))
    ) {
        runForm.value.browserSessionId = undefined;
        if (runForm.value.stateSourceType === "session") {
            runForm.value.stateSourceType = "none";
        }
    }
    if (
        recordingForm.value.browserSessionId &&
        !sessionIds.has(normalizeIdValue(recordingForm.value.browserSessionId))
    ) {
...
function applyRuntimeProfileQuickImport() {
    ...
    if (!cookies.length) {
        ElMessage.error(
            "未识别到可用 Cookie，请粘贴 Cookie 或 Set-Cookie 内容",
        );
        return;
    }
...
function buildRuntimeProfilePayload() {
    ...
    const variables =
        parseOptionalJsonObject(
            runtimeProfileForm.value.variablesText,
            "变量",
        ) || {};
    const cookieRules =
        parseOptionalJsonArray(
    ...
    request
        .then((response) => {
            ...
            return loadRuntimeProfiles().then(() => {
                ...
                runtimeProfileForm.value = normalizeRuntimeProfile(
                    hit || saved,
                );
            });
        })
...
function handleCaseSelectionChange(selection) {
    selectedCaseRows.value = Array.isArray(selection)
...
function openBatchRunDialog() {
    const candidates = (
        Array.isArray(selectedCaseRows.value) ? selectedCaseRows.value : []
    )
...
async function submitRun() {
    ...
    try {
        ...
        activeTab.value = "run";
        runQueryParams.value.webCaseId =
            runTargets.length === 1 ? runTargets[0].webCaseId : undefined;
        getRunList();
