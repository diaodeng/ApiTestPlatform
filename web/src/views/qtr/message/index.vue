<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      ...
      <el-form-item :label="dataName+'ID'" prop="messageId">
        <el-input
    ...
    >
      ...
      <el-table-column :label="dataName+'ID'" prop="messageId" width="150px"/>
...
<script setup>
import {listMessage} from "@/api/qtr/message";
  ...
  listMessage(queryParams.value).then(response => {
    ...
    nextTick(() => {
      ...
      if (allSelectIds.value && allSelectIds.value.length > 0) {
        if (tableRef.value && messageList.value) {
          messageList.value.forEach(item => {
            if (allSelectIds.value.some(dataId => dataId === item.messageId)) {
...
function handleSelect(selection, row) {
  let pageIds = messageList.value.map(item => item.messageId);
  let pageSelectIds = selection.map(item => item.messageId);
  let delIds = pageIds.filter(item=> !pageSelectIds.includes(item));
  pageSelectIds.forEach((dataId) => {
    if (!allSelectIds.value.some((messageId) => messageId === dataId)) {
...
function handleSelectionChange(selection) {
  ...
  // let pageIds = messageList.value.map(item => item.messageId);
  // 从选中数据中排除当前页的数据
  // allSelectIds.value = allSelectIds.value.filter((dataId) => !messageList.value.some(item=>item.messageId === dataId));

  // 添加当前页选中的数据
  // selection.forEach((item) =>{
  //   if (!allSelectIds.value.some((messageId)=>messageId === item.messageId)){
  //     allSelectIds.value.push(item.messageId);
  //   }
  // });
...
function handleSelectAll(selection) {
  ...
  if (pageSelectIds && pageSelectIds.length > 0) {
    pageSelectIds.forEach((item) => {
      if (!allSelectIds.value.some((messageId) => messageId === item)) {
