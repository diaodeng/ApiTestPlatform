<template>
  <el-dialog v-model="open" :title="form.profileId ? '编辑采集服务' : '新增采集服务'" width="640px" append-to-body>
    <el-form :model="form" label-width="130px">
      ...
      <el-form-item label="推送地址" :required="form.enabled">
        <el-input v-model="form.pushUrl" placeholder="例如：https://vmagent.example.com/api/v1/import/prometheus" />
      </el-form-item>
      <el-form-item label="认证用户">
        <el-input v-model="form.authUser" placeholder="接收端 Basic 认证用户，可留空" />
      </el-form-item>
      <el-form-item label="认证密码">
        <el-input v-model="form.authPassword" type="password" show-password :placeholder="passwordPlaceholder" autocomplete="new-password" />
      ...
      <el-row :gutter="16">
        <el-col :span="8">
          <el-form-item label="推送间隔(秒)">
            <el-input-number v-model="form.intervalSeconds" :min="1" :max="3600" controls-position="right" style="width: 100%" />
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="批次条数">
            <el-input-number v-model="form.batchSize" :min="1" :max="10000" controls-position="right" style="width: 100%" />
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="超时(秒)">
            <el-input-number v-model="form.timeoutSeconds" :min="1" :max="60" controls-position="right" style="width: 100%" />
          </el-form-item>
        </el-col>
      </el-row>
...
function save() {
  ...
  saving.value = true
  const request = form.profileId ? updateMetricsCollector(form.profileId, form) : addMetricsCollector(form)
  request
    .then(() => {
