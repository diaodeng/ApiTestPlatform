...
- 话题锚点（群消息 ID）与回帖记录持久保存在工单同步元数据中：外部同步更新、多维表格拉取等后续链路刷新工单时不会丢失锚点，只要工单推送过群消息，之后任何一次 AI 分析完成都能正常回帖。2026-09-10 之前存在锚点被后续同步更新静默擦除的缺陷（表现为"群里能看到工单消息但收不到 AI 结果回帖，服务端日志出现'工单无群消息锚点'跳过"），已修复；缺陷期间被擦除锚点的存量工单无法自动补发，可按下方"如何手动补发 AI 结果回帖"操作。
