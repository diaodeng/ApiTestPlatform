...
**方式一：重新提交一次 AI 分析并指定本次回帖（推荐，结果与锚点都新鲜）**

1. 进入「工单管理」打开目标工单详情页，切换到「AI 分析」页签。
2. 点击「提交 AI 分析任务」，在弹出的表单中找到「结果回帖工单群」下拉，选择**「本次回帖」**（该选项会跳过全局开关，但推送时机 sendOn 仍需配置结构有效；不确定时保持「跟随全局配置」并先确认 ⑧ 卡片中「启用回帖」已开启、推送时机为「仅分析成功」或「成功和失败都推送」）。
3. 提交后等待分析完成，结果会自动回帖到该工单在工单群的话题下。
4. 注意：若该工单此前从未推送过群消息（无话题锚点）且「无锚点处理」为「跳过」，本次回帖仍会被跳过——此时先把「无锚点处理」改为「先补发再回帖」，或在同步自动化页用方式二先建立话题。

**方式二：手动按工单号发送群消息建立话题，再等下一次分析回帖**

1. 进入「同步自动化」配置页 →「入库流程」→「⑧ 工单群消息推送」卡片区域的手动发送入口（或调用 `POST /ticket/sync/notify/group/send-by-ticket` 接口，需要 `ticket:sync:config:edit` 权限），按工单号发送一条工单信息消息。
2. 发送成功后系统会记录新的话题锚点，之后该工单的 AI 分析完成即可正常回帖。

**注意**：手动补发依赖工单在群里的既有话题锚点或新建话题；已删除的群话题、已撤回的群消息无法作为回帖目标。回帖失败不影响工单分析结果本身，服务端日志会记录具体跳过原因（可按工单号搜索"AI结果话题回帖跳过"）。
