...
因此，同一轮有效 AI 提取到新门店时可以更新 `storeId`，但不会改变 `sourceStoreCode`。只有最终 `storeId` 会作为日志接口门店参与商家和 `org_no` 校验。任务级明确参数和当前有效同步字段优先，历史 hints 负责补齐缺失参数；识别审计只记录本次运行，不作为下一次配置来源。
