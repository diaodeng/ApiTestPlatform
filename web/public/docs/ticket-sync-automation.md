...
入口：工单管理 -> 工单同步配置 -> `自定义统计` 页签。

该能力从当前系统 `ticket` 数据统计，不读取飞书群消息，也不会写入 `ticket_statistics_*` 快照表。统计方案保存在同步自动化配置中，统计结果只在本次手动执行响应或通知消息中出现。

#### 统计方案

- `profileCode`
  - 方案唯一编码，仅允许英文、数字、下划线和中划线。
  - 定时任务通过该编码指定执行方案；留空时执行所有启用方案。
- `label`
  - 方案显示名称，也会作为通知标题。
- `enabled`
  - 停用的方案不会被手动“全部执行”或定时任务执行。
- `timeField`
  - 统计范围所依据的工单时间字段：提交时间、创建时间、首次响应时间、形成结论时间、处置完成时间、关闭时间。
- `timeRange`
  - 可选今天、昨天、最近 N 天、本周、上周或固定自定义范围。
  - 所有范围使用上海时区，查询区间为“开始时间包含、结束时间不包含”。今天和本周统计到任务实际执行时刻；昨天和上周统计完整自然时间段。
- `scope`
  - 可按项目 ID、模块 ID、模块编码、工单类型过滤；不填写表示不限制该维度。

#### 分组方式

- 按字段原值分组：直接按工单状态、工单类型、根因、解决方式、模块等字段统计。
- 按条件规则归类：每个分组可配置多个条件，并选择“全部满足”或“任一满足”。
  - 可选操作符：等于、包含、属于、正则、为空、不为空。
  - 条件字段由后端白名单限制，不能执行 SQL 或 Python 表达式。
  - `exclusive` 表示一张工单只归入优先级最高的一个命中分组；允许重叠时可同时计入多个分组。
  - 未命中工单可归到“未归类”，也可仅计入总数而不显示在分组中。

“是否有结论”对应 `hasConclusion` 派生字段，其规则是工单 `processed_at`（首次形成处理结论时间）是否为空。推荐用规则分组配置“有结论 = hasConclusion 等于 true”“无结论 = hasConclusion 等于 false”，不要仅使用当前状态替代结论口径。

#### 通知配置

- `sendMode`
  - `push_config`：使用系统推送配置。
  - `feishu_app`：使用飞书应用身份向指定 `chat_id` 发送。
  - `hybrid`：两种渠道同时发送；其中一个渠道未配置时仍会使用另一个可用渠道。
- 飞书凭证
  - 方案可单独填写 `appId/appSecret`；留空时自动继承页面中的统一 `feishuAuth` 凭证。
- `messageFormat`
  - 文本模板：所有渠道发送相同文本。
  - 飞书卡片：飞书应用渠道发送交互卡片，推送配置渠道仍发送相同文本。
- `messageTemplate`
  - 常用变量：`${profile_label}`、`${start_time}`、`${end_time}`、`${time_field_label}`、`${total_count}`、`${group_summary}`、`${top_tickets}`、`${now_time}`。
  - 留空使用系统默认模板。
- 工单明细
  - 可选择附带最新的前 N 条工单，N 的可选范围为 1 到 20；默认不发送明细，避免通知过长或泄露不必要的信息。

#### 手动与定时执行

- `操作` 页签可以选择一个或多个方案执行；关闭“发送通知”时只返回预览结果。
- 手动接口：`POST /ticket/sync/custom-statistics/run`。
- 字段定义接口：`GET /ticket/sync/custom-statistics/definitions`。
- 定时任务名称：`module_task.scheduler_maintenance.ticket_custom_statistics_report`。
  - 任务参数 `profileCodes`：方案编码数组或逗号分隔字符串，可选。
  - 任务参数 `startTime/endTime`：可选统一时间覆盖，必须同时传入，例如 `{"profileCodes":["daily_coupon_conclusion"],"startTime":"2026-08-04T00:00:00","endTime":"2026-08-05T00:00:00"}`。

#### 注意事项

- 不落快照意味着历史区间按“当前工单字段值”重新计算。工单之后被改状态、补结论或修正分类时，重新执行同一历史区间可能得到不同结果。
- 单个方案最多扫描 50,000 条工单。超出时任务会明确失败，需缩小时间范围或增加过滤条件。
- 定时任务只保存运行状态和发送摘要，不返回完整分组明细，避免把统计内容写进任务运行日志。
...
1. 管理员在“自定义统计”页签创建并保存一个或多个统计方案。
2. 手动执行时可选择方案并选择“仅预览”或“按方案通知”；定时执行时使用 `module_task.scheduler_maintenance.ticket_custom_statistics_report`。
3. 服务按方案的单一时间字段、时间范围和 SQL 范围过滤查询当前 `ticket` 数据。
4. 服务在内存中按字段或安全规则聚合，再按方案模板渲染文本或飞书卡片。
5. 结果不会写入统计快照；调度器仅保留方案编码、总数和通知发送摘要等运行信息。
