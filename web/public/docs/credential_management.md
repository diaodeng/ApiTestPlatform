...
编辑弹窗顶部以**卡片**选择获取方式（手工录入 / HTTP 自动登录 / HTTP 刷新 / 浏览器人工登录 / 浏览器刷新），每张卡片带场景说明；"生命周期与自动维护"区会实时显示当前的**生命周期策略**：人工维护、仅手工获取、定时重新认证（HTTP 登录 + 自动维护）、刷新优先登录兜底（HTTP 刷新 + 自动维护），并提示认证成功后实际保存的凭证内容（响应提取目标或认证流程写回输出）。
...
选择"HTTP 登录"时可配置用户名、密码、OTP 类型和登录接口。**选择"HTTP 刷新"时同样会显示登录账号配置区**——此时账号密码供兜底登录接口使用：刷新用现有凭证续期不需要账号，但刷新失败后的自动登录需要账密（可带 TOTP）。

- **用户名/密码**：目标系统的登录账号和密码，由用户自行填写，加密保存。**账密必须填在登录账号区，不要写在登录/兜底接口的请求体里**——步骤配置中的明文密码保存后会被脱敏，且回填时无法还原，后续编辑会导致登录失败。在请求模板和多步链步骤中通过 `${secret.username}` 和 `${secret.password}` 引用（接口参数名与变量名是两回事，例如 rta-os 的参数应写 `"pwd": "${secret.password}"`）。账号信息仅用于登录流程，业务使用凭证时只投影 Header/Cookie，不会携带账号信息。
...
- **变量引用校验**：单步模板与多步链步骤中引用的 `${secret.字段名}` 必须在凭证中真实存在（前后端双重校验），引用不存在的字段（如误写 `${secret.pwd}` 而凭证字段名是 `password`）保存时会明确拒绝，避免执行期占位符原样发出导致登录失败。

### 请求模板占位符

登录/刷新请求通过"请求体类型"选择请求体的形态：**JSON**（application/json）、**表单**（application/x-www-form-urlencoded 键值对）或**无请求体**（GET 等），选择后只显示对应的编辑器。占位符在所选编辑器中发送前自动替换：
...
页面会在 Header、查询参数和当前所选请求体的编辑器旁提供“插入变量”按钮。光标位于 JSON 字符串外时，按钮会自动补全双引号；位于已有字符串内或选中原值时，会直接插入/替换变量，例如 `"${secret.cookie}"`。保存前会校验变量格式、未闭合占位符以及当前凭证是否存在被引用的字段；不支持 `${secret.header.cookie}` 这类嵌套路径。JSON 与表单为互斥选择，切换类型时另一端的编辑内容会被清空（重新选择回原类型不会恢复，请切换前确认）；编辑已有凭证时，系统按已保存模板自动推断类型。
...
**入口**：编辑弹窗选择”HTTP 登录重新获取”后，登录接口配置区有”**多步认证链**”开关；打开后展示认证流程预览和步骤编辑卡片。”HTTP 刷新”模式的**刷新接口段**与**兜底登录接口段**同样各有独立的”多步认证链”开关：刷新段开启配置多步刷新链（`refreshSteps`），兜底段开启配置多步兜底登录链（`loginSteps`，与 HTTP 登录共用同一份配置）——兜底登录因此同样支持 TOTP 自动验证、multipart 提交与条件步骤。步骤支持添加、删除、上移、下移；每段提供”**测试登录/刷新流程**”按钮（已保存的凭证）真实执行整条链并弹出逐步结果，**测试过程不写回凭证**。
...
**手动登录与空凭证**：录制或执行时如果开启了"手动登录"，凭证内容（storageState）允许为空——首次录制/首次执行时浏览器状态可能还没有产生，此时凭证仅作为"目标站点 + 回写目标"的引用，登录完成后的最终浏览器状态由 Agent 上报，可显式保存为新凭证或回写到原凭证。未开启手动登录时，选择的浏览器凭证必须已有 storageState 内容，否则启动会报"浏览器凭证未配置 storageState"（此时浏览器状态只能来自凭证，无法人工补录）。
