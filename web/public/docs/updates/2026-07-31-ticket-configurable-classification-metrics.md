...
自定义趋势指标由管理员维护指标、分组和条件。允许的字段由后端白名单控制，避免配置参与任意 SQL；每个条件可使用 `equals`、`contains`、`in`、`regex`、`is_empty`、`is_not_empty`，分组支持全部满足或任一满足，指标支持分组重叠或互斥。`is_empty` 匹配 `null`、空字符串和全空白字符串，`is_not_empty` 匹配其余值，二者不填写匹配值；文本 `null` 仍是普通字符串。条件字段为“工单类型”时，匹配值下拉会展示当前工单类型配置，仍允许手动补充编码。统计页会默认选中全部已启用指标，用户可在“自定义趋势”中取消不需要的指标后再查询。
