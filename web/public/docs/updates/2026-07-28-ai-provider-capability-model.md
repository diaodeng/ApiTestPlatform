...
1. 新增或编辑 Provider 时可点击“更新模型”。请求始终使用当前表单中的地址、协议和协议配置，不写入数据库；新增必须填写页面密钥，编辑未填写新密钥时仅在服务端临时读取同一 Provider 的已保存密文密钥。
...
4. 默认模型旁的“测试”按钮同样使用当前表单草稿向默认模型发送最小请求。测试成功仅展示模型输出摘要，服务端日志不会记录密钥。
