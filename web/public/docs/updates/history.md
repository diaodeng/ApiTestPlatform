...
## 2026-09-01
- 日志内存分析全面升级：扫描引擎复用 rg 搜索管道（保留降级与保护），数据点携带行号/epoch 并增加解析统计；弹窗内容区可滚动、内存面板可最小化、点日志行自动最小化；图表与日志双向联动（点曲线跳日志行、点日志行画标记线）；日志拉取记录管理页新增"曲线"独立入口（曲线+数据点列表，点击跳日志）。详见：[内存分析升级与双向联动](2026-09-01-ticket-log-resource-curve-linkage.md)。
