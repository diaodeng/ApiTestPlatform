...
## 2026-09-03
- 工单模块映射大小写回归修复：8/28 起新入库工单 module_id/module_code 全部为空，根因是 8/27 关键字归一化统一小写后，模块映射匹配的工单文本未同步小写导致映射永远无法命中（手动多维表格拉取只是触发面）；现已修复并补回归测试，同日已执行模块映射修复脚本回填 1045 条存量工单，剩余约 725 条需补齐映射配置后重跑。详见：[工单模块映射大小写回归修复](2026-09-03-ticket-module-mapping-case-fix.md)。
