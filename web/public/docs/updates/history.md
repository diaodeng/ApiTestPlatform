...
2. 新增说明文档：`web/public/docs/updates/2026-08-08-ticket-log-viewer-column-resize-handle-fix.md`。

## 2026-08-07

1. 工单日志查看器搜索结果列表移除独立“时间”列，改为在“日志内容”表头显示时间排序箭头并继续按日志时间升降序排序。
2. 搜索结果虚拟表格新增自定义列宽拖拽，可按需调整文件、行号和日志内容列宽；操作列仍保持固定宽度。
3. 日志搜索关键字上限从 10 提升到 20，每个关键字最多 200 字符；结果上限保持不变，高亮关键字也同步提升到 20 个，并按关键字轮换不同颜色。

## 2026-08-05

1. 修复工单日志查看器搜索结果中的“日志时间”排序：点击表头可直接切换升序和降序，不再依赖虚拟表格的排序事件。

## 2026-08-03

1. 工单日志查看弹窗的搜索结果列表切换为 `el-table-v2` 虚拟表格，保留点行加载上下文和“在此文件搜索”动作，用于缓解大结果集滚动卡顿。
2. 新增说明文档：`web/public/docs/2026-08-03-ticket-log-viewer-search-result-virtual-table.md`。
3. 日志内容列补充单行省略样式，避免虚拟表格内长文本换行后撑高行高。

## 2026-07-31

1. 同步自动化页的“自动分类管理”新增“映射归类”策略，管理员可直接使用工单已保存的原始外部字段快照重跑工单类型映射。
2. 映射归类不会调用 AI，并在页面明确说明人工确认类型默认受保护；只有显式勾选“强制覆盖已有分类”才允许覆盖人工类型。
3. 正则归类未配置规则时继续返回“未配置正则归类规则”，不再依赖空规则隐式切换到映射或 AI。
4. 更新批量重归类接口说明，统一请求字段为 `ticketNos`，补充 `external_mapping` 的数据来源、未命中语义和人工类型覆盖边界。
5. 新增说明文档：`web/public/docs/2026-07-31-ticket-external-mapping-reclassification-ui.md`。

## 2026-07-28

1. AI Provider 重构为平台、API 协议、支持用途、兼容执行器和默认模型五个独立维度；旧 `provider_type` 不再保留兼容。
2. Provider 默认模型支持远端目录更新、可搜索选择和手工输入；密钥默认不回显，查看时需要单独权限和当前用户密码二次验证。
...
1. 工单日志查看弹窗在日志准备完成后读取当前日志拉取记录的完整文件列表，文件范围下拉在初次打开和搜索后均保留后端返回的全部文件及顺序，不再仅显示搜索命中文件。
2. 日志搜索结果区或日志详情区全屏时，按 `Esc` 仅还原全屏子区域；两个区域均非全屏时，`Esc` 保持关闭日志查看弹窗的原有行为。
3. 新增说明文档：`web/public/docs/2026-07-28-ticket-log-viewer-file-list-and-escape.md`。
...
1. 工单 AI 分析整包日志新增 Agent 本地缓存：同一工单、同一成功日志记录、同一日志来源再次分析时复用已下载并解压的日志，避免反复下载。
