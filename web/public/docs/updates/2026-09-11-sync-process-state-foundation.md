...
**部署顺序**：本批 DDL 可随阶段一窗口执行 → 执行统一迁移脚本 `sql/20260911_migrate_extra_data_state_to_tables.sql`（三段合一：锚点表/任务表幂等列/宽表，幂等可重跑，已对生产库完成只读 preview 验证：11 条锚点、3 条幂等记录、2507 个状态工单；可替代 Python 版 `scripts/migrate_group_push_anchor.py`，择一执行）→ 部署服务切换版本（顺序不可颠倒）。
