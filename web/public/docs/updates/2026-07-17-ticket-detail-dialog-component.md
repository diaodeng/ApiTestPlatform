...
9. 二次收口后，tab 子组件不再接收父级上下文对象。评论、历史只接收 `ticketId/active`；概览、日志拉取、协同支持 `ticketId/detail/active` 双入口，父详情已持有 `detail` 时直接复用详情，未传详情时才按 `ticketId` 自行请求接口。
...
- `TicketDetailOverviewTab.vue`：优先使用外部传入的 `detail` 展示最新 AI 结论、快照和相似工单；未传详情时通过 `ticketId` 拉取，并自行完成相似工单归因。
- `TicketDetailLogPullTab.vue`：优先使用外部传入的 `detail` 作为工单元信息；未传详情时通过 `ticketId` 拉取，同时自行维护日志拉取记录、日志提交表单和日志查看器数据。
- `TicketDetailCollabTab.vue`：优先使用外部传入的 `detail` 展示协同消息、快照和相似工单；未传详情时通过 `ticketId` 拉取，并自行完成消息、快照、知识提炼和相似归因。
