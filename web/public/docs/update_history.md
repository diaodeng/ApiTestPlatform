## 2026-06-13
1. ??????????? @ ???`sync_external_ticket` ???????????? `report_at` / `reporter_at` / `assignee_at` / `mention_at`????????????????????????????????????? @?
2. ??????????????`ticket.sync.automation.externalSyncRequiredFields` ??????? `/ticket/sync/external` ????????????????

## 2026-06-13
1. 按人催办消息新增 `rowsMarkdownTemplate` 明细行模板配置：`rows_markdown` 仍作为整段变量输出，但每条超时记录的 Markdown 样式现在可以单独配置，支持自定义序号、时间、工单号和详情链接的拼接格式；空配置时回退到默认样式。
1. 前端同步配置页新增“明细行模板”输入框，并在保存/加载时一并持久化到 `personReminder.rowsMarkdownTemplate`。
