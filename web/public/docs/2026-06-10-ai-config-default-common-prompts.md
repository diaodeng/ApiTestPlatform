# 2026-06-10 AI配置中心默认提示词补齐（自动分类 / 日志参数提取）

## 背景

AI 配置中心中“自动分类提示词”和“日志参数提取提示词”依赖 `common` 分类提示词模板。
...
- `ticket_category_classify_default`
  - 名称：工单自动分类默认提示词
  - 分类：`common`
...
- AI 配置汇总接口读取配置时，若以下字段为空字符串会自动回退默认编码：
  - `ticket.ai.category.classify.prompt.code` -> `ticket_category_classify_default`
