...
| 运行 | 打开运行确认弹窗，点「开始运行」后**立即创建一次运行并同步等待执行完成**（可能持续数分钟），期间可到运行记录 Tab 看进度。支持运行级参数：**步骤超时(ms)**（单步最长执行时间，默认 10000）、**步骤等待(ms)**（每步缓冲等待，默认 0），二者留空时用步骤自身配置；**应用方式**（作为默认值=仅对未单独设置的步骤生效；覆盖所有步骤=忽略步骤自身配置，适合整体放宽调试）；**失败策略**（失败后停止=默认；继续执行后续阶段=失败阶段照常标记，下游依赖上游产出时请自行评估）；**回写凭证**（执行结束后把浏览器最终登录态回写到版本绑定的凭证，需绑定开启"允许回写"且非共享只读，默认关闭）；**强制刷新登录态**（忽略 Agent 本地缓存的浏览器状态，用本次凭证合并的登录态初始化，凭证更新后建议开启） | **是** |
| 凭证映射 | 打开"系统凭证映射"弹窗：维护任务级"系统标识 → 凭证绑定"清单（如 erp/oms 各绑一个凭证）。映射是环境配置，**独立于版本快照，发布后仍可修改**；配合阶段切分中的"目标系统"使用（见下文"阶段化执行与多系统凭证"） | 否 |
...
每个阶段还可以声明**目标系统**（下拉选项来自任务的"凭证映射"清单，可留空）。声明后的多系统任务在运行时：

1. 运行创建时按阶段收集目标系统 → 查任务凭证映射取各系统凭证登录态 → **按域合并**为浏览器初始化状态 → 一次运行多个系统免登录；
2. 阶段声明的系统在映射中缺失/未绑定时，运行会被阻断并提示具体阶段与系统；
3. 两个不同凭证出现同域登录态（如同一站点两套账号）视为冲突，运行被阻断并列出来源——同域多账号无法在一次浏览器会话共存，这是浏览器语义限制；
4. 凭证更新后直接改凭证映射即可（发布版本无需变更），运行时开"强制刷新登录态"忽略 Agent 本地缓存的旧状态。
