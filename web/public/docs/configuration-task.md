...
| 模式 | 步骤参数 | 说明 |
|---|---|---|
| 资源绑定 | `resourceIds` | 下拉选择「资源管理」页面上传到任务执行 Agent 的资源（仅显示 `READY` 状态）；也可不选，运行前在「输入绑定」里把 `fileKey` 关联到资源 ID，输入绑定优先级更高 |
| Agent 目录文件 | `agentPath` | 下拉浏览执行 Agent 受控上传目录（`storage/upload_inputs/`）内的文件，选择受控相对路径；文件由使用方自行放置到该 Agent 目录并保证存在 |

- 两种模式互斥，切换时会清空另一模式的取值；`fileKey` 是运行时输入绑定的关联键，默认由录制自动生成（如 `file_1`），可修改；
- 解析优先级：输入绑定（`fileKey` → 资源 ID）> 步骤参数 `resourceIds` > 步骤参数 `agentPath`；
...
- `inputBindings` 的键是步骤里 `upload_file` 使用的 `fileKey`，值是资源 ID 字符串数组，每个键最多绑定 20 个资源；资源 ID 不是 Agent 本地路径，资源必须处于 `READY` 且属于任务执行 Agent；
