...
- **可视化步骤**：表格支持列表内直接编辑（点击单元格即可修改动作类型、步骤名称、首选定位器和输入参数），以及新增、前插、上移/下移、删除、启停；点击行选中后可用「编辑当前步骤」，或点行内「编辑」打开步骤详情编辑弹窗（支持动作类型、定位器、参数、断言、目标快照等完整编辑能力）；存在截图步骤时表格自动显示"证据"列；「上传文件」动作只填 `fileKey`，配套在输入绑定里把 fileKey 关联到资源 ID；
