...
轻量 AI 的配置集中在 **系统管理 → AI配置中心**（路由：`/system/aiconfig`）。页面说明见 [AI 配置中心说明](ai_config_center.md)。
...
| Agent 并发数 | 单个 Agent 同时允许执行的重型 AI 任务数量 | `1` |

当 Agent 并发数大于 `1` 时，任务会按提交顺序排队等待空闲槽位，不会因为达到上限直接失败。
