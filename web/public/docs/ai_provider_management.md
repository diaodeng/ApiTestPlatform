...
- 任务级上报始终包含：最终渲染的提示词全文、分析结果全文、Token 用量、耗时、模型名、任务/工单 ID、成功或失败状态。
- 任务级 span 由执行 Agent 的机器直连可观测平台上报（服务端只负责下发配置）：部分环境的服务端与可观测平台网络隔离（如测试服务端访问生产平台），而 Agent 机器天然可达平台，CLI 原生遥测同样从 Agent 侧上报。
...
- **网络可达性**：任务级上报和 CLI 原生遥测都由执行 Agent 的机器发起，因此 Agent 机器必须能访问 OTLP 端点；服务端无需出网访问可观测平台（即使服务端与平台网络隔离也不影响上报）。
