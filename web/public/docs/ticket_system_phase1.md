...
- AI 分析：支持按工单项目版本映射到仓库/分支，提交 AI 分析任务后由 Codex Worker 执行并回写 `ticket.ai_analysis`、`ticket_rca` 和 `ticket_event`。
...
- 日志拉取约束：日志查看改为弹窗模式，默认展示入库内容；切换到原始文档后可显示“本次截取范围”并按当前记录实时重截；提交拉取任务改为弹窗；参数配置说明改为通用提示按钮；日志时间范围必填，支持开始/结束时间或时间点前后分钟范围；日志解析仅处理 `*_pos.log*` 文件；日志内容默认不换行，支持开关切换换行显示；后端仅传压缩结果，前端使用 `decompressText` 解压展示；入库内容原样保存，超过上限时直接失败并提示缩小时间范围。
...
- 已实现：AI 仓库映射管理（已独立成菜单）、AI 分析任务提交、Codex CLI 后台执行、结果回写工单和 RCA、历史任务查询。
...
- 2026-05-20：日志查看改为弹窗并支持原始文档实时重截；日志时间范围改为必填并支持时间点前后范围；日志解析仅处理 `*_pos.log*` 文件。
