...
2. 如果映射未配置 `localRepoPath`，或 `localRepoPath` 当前分支与 `branchName` 不一致但存在 `repoUrl + branchName`，Agent 会在 AI 工作区下创建或复用固定 Git worktree：
...
5. 如果 Git 未安装、`repoUrl/branchName` 缺失、自动 worktree 创建失败，Agent 会失败并返回明确原因。

## 使用建议

- 常用分支建议在仓库映射里维护固定 `localRepoPath`，例如每个分支一个 worktree 目录。
- 不建议多个分支共用同一个普通 clone 目录。
- 如果历史映射把多个版本都指向同一个 `localRepoPath`，现在不会再直接使用错误分支目录；Agent 会自动转到按分支隔离的 worktree。
