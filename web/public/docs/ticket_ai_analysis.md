# 工单深度 AI 分析说明

## 功能

工单详情页的“发起 AI 分析”用于结合工单、时间线、日志和指定版本仓库生成根因分析、证据、风险和处理建议。任务由本机 Agent 执行，服务端负责结果校验和写回工单。

入口：工单详情页 → AI 区域 → **发起 AI 分析**。提交后可在“任务历史”查看状态、执行 Agent、模型、工作区和错误信息。

## Provider 与执行器

- Codex：结果从任务工作区的 `result.json` 读取。只有 Worker 退出码为 `0`、结果 JSON 可解析且通过本次任务的 JSON Schema 时才算成功。
- Claude Code：使用命令输出的 JSON，优先读取 `structured_output`，其次解析 `result` 最终文本；`is_error=true` 或进程执行失败时算失败。
- Worker 的 `stderr` 可能包含正常进度、模型输出和诊断信息。`stderr` 非空本身不会使 Codex 分析失败。
- Worker 失败时，任务会记录结构化的“错误码”和真实“错误信息”；失败响应不包含工单正文或分析结果。`workerExitCode` 仅用于辅助诊断，不能替代业务错误码。
- Schema 兼容接口中按字符串传输的 BIGINT ID，并包含 `symptom`、`similar_cases`、`sop_suggestion`、`owner_suggestion`、`monitoring_suggestion` 等可选增强字段；字段缺省时由服务端补默认值。
- 增强字段（含 `similar_cases`）接受数组或叙述字符串两种输出：模型写成叙述文字时不会导致校验失败，服务端归一化会自动把字符串包装为单元素数组再写回结果。
- 分析结果未通过 JSON Schema 校验时，任务失败信息会包含具体违规字段明细（如 `$.similar_cases: 期望 array，实际 string`），无需人工比对结果文件定位。
- 模型输出 JSON 时字符串值内部可能带未转义的英文双引号（如中文叙述里引用术语，甚至原样嵌入请求体 JSON 示例），会导致整体 JSON 解析失败。系统按以下顺序自动挽救：
  1. **回溯式 JSON 修复**：按"键/值字符串上下文 + 嵌入花括号剪枝"智能判断每个歧义引号是字符串结束符还是值内字符（含转义），可修复单遍扫描无法处理的嵌套 JSON 示例形态；
  2. **Agent 自动补救重试**：修复仍失败时，自动保留首次执行现场（`result.attempt1.json` 等文件），resume 同一会话发送纠错指令让模型重新输出一次（通常一轮即可纠正）；重试成功时两轮 Token 消耗合并计入审计，任务消息会注明"经自动补救重试成功"。
  两道防线都失败才判定任务失败，失败信息会包含输出预览摘要（`AI_WORKER_RESULT_UNPARSEABLE`），在任务列表即可看到具体断点。提示词也已要求模型使用中文引号或转义双引号，从源头减少该问题。

## Token 用量统计

- Token 用量在 Agent 客户端本地统计完成后，仅把最终汇总值随结果回传服务端；过程事件流数据不会上传。
- Codex 以 `--json` 事件流模式运行：每回合结束上报一次用量，客户端把所有回合累加得到整个任务的总量，而不是某一次回合的值。
- Claude Code 从结果 JSON 的 `modelUsage` 按实际使用的模型逐个累加（含辅助小模型的消耗），比顶层 `usage`（仅主模型最后一次调用）更准确。
- 任务记录中的 Token 字段：
  - `inputTokenCount`：输入 Token 累计（含缓存命中）。
  - `outputTokenCount`：输出 Token 累计。
  - `totalTokenCount`：输入 + 输出总量。
- 审计执行记录额外保存缓存命中明细（Codex 的 `cachedInputTokens` / Claude 的缓存读写），缓存部分不重复计入总量。
- Token 用量解析失败不影响分析结果：此时任务照常成功，Token 字段为空。
- 失败的任务如果模型调用已经发生（例如 Worker 执行到一半失败、结果未通过校验、执行超时），系统会尽力从已完成回合的事件流、结果文件或工作区日志中提取已消耗的 Token 并写入审计记录；确实无法提取时字段为空，表示“消耗未知”。
- 缓存复用的任务（重试时直接返回 `result.json` 中的历史结果）同样会尽力恢复当时的 Token 用量。
- 历史任务（修复前执行）没有用量数据，显示为 0 属正常现象。

## 可观测上报

如果当前 Provider 在 AI Provider 管理中开启了"可观测上报"，每次分析完成后会把任务级调用（完整提示词、分析结果、Token 用量、耗时、成功/失败）上报到可观测平台，可在平台 Traces 页按 `service.name` 筛选查看；开启"CLI 原生遥测"后还能看到本机 Codex / Claude Code 的细粒度执行链路。配置方法与注意事项见 [AI Provider 管理说明](ai_provider_management.md) 的"可观测上报（OTLP）"章节。上报为旁路能力，平台不可达不影响分析任务。

## 服务重启与断连恢复

服务端发布重启期间，本机 Agent 的执行不会中断，结果也不会丢失：

- Agent 执行完成后，如果服务端暂时连不上（正在发布重启），响应会暂存在 Agent 本地的待补交清单中，服务端恢复连接后自动补交。
- 服务端对重启前发起、重启后才返回的结果会保留 24 小时；启动恢复时发现结果已就绪的任务会直接写回成功（状态描述为“分析成功（恢复服务重启前的执行结果）”），不需要重新调用模型，也不需要人工重试。
- 只有在结果缓存过期（超过 24 小时）或结果确实不完整时，任务才会标记为“服务重启前任务未完成，已清理为失败”，此时重试即可。
- Agent 配置页提供“断线重连”开关和“低频间隔”设置：开启后，高频重试次数用完仍连不上服务端时，Agent 会按低频间隔（默认 300 秒）持续重连直到手动停止，保证发布重启时间较长时系统仍能自愈。该开关默认关闭，默认行为与旧版本一致。

## 执行中连接中断的自动恢复（2026-09-14 起）

AI 分析执行过程中，如果 Agent 与服务端的连接中断（例如客户端机器睡眠、网络切换、VPN 断开），任务**不会被判定为失败**，而是进入“恢复中（等待Agent补交）”状态自动等待恢复：

- 连接中断时任务显示“恢复中（等待Agent补交）”，工单列表处理状态显示“AI恢复中”，不会发送失败通知（网络闪断不是真实失败）。
- Agent 端的分析 Worker 在后台线程继续执行，不受连接中断影响；执行完成后结果暂存在 Agent 本地，重连成功后自动补交。
- 服务端恢复扫描任务（默认每 60 秒）检测到补交结果后，自动把结果写回任务并更新为成功，**不需要重新调用模型，也不需要人工重试**。
- 如果 Agent 长时间没有重连（超过恢复期限，约为任务超时时间加 15 分钟），任务才会标记为失败（原因“Agent 未在恢复期限内补交分析结果”）并发送失败通知，此时重新发起分析即可。
- 恢复等待期间可以手动取消任务（取消后补交的迟到结果不再写回）；重试按钮在恢复期间不可用，如需立即重新执行请先取消再重新发起。

### Agent 连接健康自检（客户端）

新版客户端 Agent 内置心跳看门狗：服务端每 30 秒发送一次心跳，如果客户端超过 90 秒没有收到服务端任何消息，会判定连接已经静默死亡，主动断开并自动重连（走“断线重连”既有策略）。这解决了此前“连接已断但界面仍显示运行中、也不会自动重连”的问题（例如电脑睡眠唤醒后连接已失效）。服务端侧超过 2 分钟未收到 Agent 心跳也会将其判定为离线并更新状态，保证页面显示的 Agent 在线状态真实可靠。

### Agent 离线时提交 AI 分析

如果提交 AI 分析时所选 Agent 已离线，任务会**立即失败**并提示“Agent 当前离线，AI 分析任务未下发”，不会像旧版本一样在“分析中”状态停留约一小时才超时。请确认客户端 Agent 已连接（必要时在 Agent 页面手动断开后重连）再重新发起。

## 请求幂等

服务端会根据以下输入生成分析请求指纹：

- 工单、版本和日志来源记录；
- 仓库映射、仓库地址和分支；
- 最终提示词和输出 Schema；
- Provider、模型、执行器和 Agent；
- 日志分析模式、时间窗口策略和 resume 选项。

同一指纹已有成功任务时，重复提交或点击重试会直接返回原成功结果，不重新调用 Agent，也不会重复写入 AI 消息、RCA、快照和事件。失败任务不占用成功结果唯一记录，因此可以继续重试。对尚未写回成功结果、但任务工作区已经有有效 `result.json` 的重试，会使用新的 Agent 网关请求 ID，并由本机 Agent 直接复用该文件，不重新启动 Worker。

启用“强制刷新”会生成新的请求指纹并重新分析。只有在日志、版本、提示词或模型等输入确实发生变化，或明确需要重新分析时才建议使用。

## 工作区与代码仓库

每次任务使用独立目录：

```text
<工作区根目录>/ticket_<工单ID>/task_<任务ID>/
```

该目录保存 `ticket.json`、`timeline.json`、`context.json`、提示词、结果和 Worker 日志。Codex 会把当前任务目录作为主项目并自动加入 trusted，代码仓库使用指定目录下的 Git worktree 作为额外可写目录。不会自动信任整个磁盘、用户目录或仓库父目录。

Codex 使用 `workspace-write` 沙箱和自动审批参数运行，适用于后台分析任务；这不等同于关闭操作系统 UAC，也不应改成 `dangerously-bypass-approvals-and-sandbox`。

## 常见问题

### 任务显示失败，但日志中有分析内容

先查看任务工作区的 `result.json`、`worker.stdout.txt` 和 `worker.stderr.txt`。如果 Codex 退出码为 `0` 且结果符合 Schema，重试会复用该任务工作区中的有效结果；如果 JSON 内容损坏、字段类型错误或缺少必填字段，仍需重新分析。

### 为什么重复点击没有产生新任务

这是请求幂等的正常表现。只要输入指纹一致且已有成功结果，系统会返回历史成功任务，避免重复消耗模型调用并避免重复写回。

### 为什么强制刷新后任务ID变化

强制刷新明确要求绕过历史成功结果，因此会创建新的任务和新的请求指纹；旧结果仍保留在任务历史中。

### 如何判断 AI 分析失败

服务端以任务响应中的 `success` 和 `status` 判断执行状态：`success=false` 或 `status=failed/timeout` 表示失败，不再通过工单正文、分析结果文本或日志中的普通 `Error:` 关键字推断失败。

失败任务可查看以下字段：

- `errorCode`：稳定的业务错误码，例如 `AI_PROVIDER_QUOTA_EXCEEDED`、`AI_PROVIDER_AUTH_FAILED`、`AI_WORKER_PERMISSION_DENIED`。
- `errorMessage`：Provider 或 Worker 返回的真实异常信息。
- `workerExitCode`：本地进程退出码，仅作为辅助信息。

如果 Worker 同时出现本地诊断告警和 Provider 致命异常，Provider 致命异常作为主错误保存，本地告警只保留在 Agent 诊断日志中。

### 结果结构轻微偏差的自动清洗

部分模型即使被输出 Schema 约束，仍可能做出两类轻微偏差：在结果中附加 Schema 之外的字段（如 `ticket_no`、`merchant_name`），或把 `evidence` 证据写成 `{source, content}` 对象数组而不是字符串数组。这类结果内容本身完好，只有结构不符合约定。

系统在校验前会做一次保守清洗：剔除 Schema 之外的额外字段、把证据对象拼接为 `"来源: 内容"` 字符串、其他非字符串证据元素转为 JSON 字符串。清洗动作会记录在 Agent 与服务端日志中（形如 `$.ticket_no: 已剔除 schema 外额外字段`），可审计；清洗后仍必须通过完整 Schema 校验，明显非法的结果依旧按失败处理。清洗只作用于内存中的结果，不会改写工作区中的原始 `result.json` 文件。
