...
from plugins.pip_pins import PLUGIN_PIP_REQUIREMENTS
...
# pip 安装模式默认包索引源（国内镜像，可在插件管理中自定义）
...
class PluginManager:
    ...
    def read_pip_index_url(self) -> str:
        """
        读取 pip 包索引源：配置了自定义源用之，否则默认国内镜像。
        :return: 包索引源地址
        """
        try:
            configured = str(PluginsConfig.read().pip_index_url or "").strip()
        except Exception as e:
            logger.warning(f"读取 pip 包索引源配置失败，回退默认源: {e}")
            configured = ""
        return configured or DEFAULT_PIP_INDEX_URL

    def save_pip_index_url(self, index_url: str) -> tuple[bool, str]:
        """
        保存 pip 包索引源配置（留空恢复默认国内镜像）。
        :param index_url: 包索引源地址
        :return: (是否成功, 结果消息)
        """
        try:
            config = PluginsConfig.read()
            config.pip_index_url = str(index_url or "").strip()
            PluginsConfig.write(config)
            logger.info(f"pip 包索引源已保存: {self.read_pip_index_url()}")
            return True, "Pip 源已保存"
        except Exception as e:
            logger.exception(f"pip 包索引源保存失败: {e}")
            return False, f"Pip 源保存失败: {e}"

    def _locate_pip_python(self) -> Path | None:
        """
        定位执行 pip 安装的 Python 解释器。

        优先使用内置嵌入式运行时（runtime/python，打包态与开发态同一路径约定），
        打包态额外兼容 PyInstaller 解压目录；最后回退当前解释器（仅当其自带 pip）。
        :return: 解释器路径，全部不可用时返回 None
        """
        candidates: list[Path] = []
        if getattr(sys, "frozen", False):
            candidates.append(_frozen_base_dir() / "runtime" / "python" / "python.exe")
            meipass = getattr(sys, "_MEIPASS", None)
            if meipass:
                candidates.append(Path(meipass) / "runtime" / "python" / "python.exe")
        else:
            candidates.append(_plugin_base_dir() / "runtime" / "python" / "python.exe")
        for candidate in candidates:
            if candidate.exists():
                return candidate

        # 回退：当前解释器自带 pip 时（如手工创建的 venv）也可直接使用
        import subprocess
        ...
        try:
            ...
            logger.debug(f"探测当前解释器 pip 能力失败: {e}")
        return None

    @staticmethod
    def _runtime_python_version(python_exe: Path) -> str:
        """
        获取目标解释器的 Python 大.小版本，用于 manifest 兼容字段。
        :param python_exe: 解释器路径
        :return: 版本字符串（如 3.11），获取失败回退当前进程版本
        """
        import subprocess
        ...
        try:
            ...
            logger.debug(f"获取运行时 Python 版本失败: {e}")
        return f"{sys.version_info.major}.{sys.version_info.minor}"

    @staticmethod
    def _check_modules_in_dir(modules: tuple[str, ...], target_dir: Path) -> list[str]:
        """
        校验插件要求的模块在目标目录中是否可定位（仅定位不导入，避免副作用）。
        :param modules: 顶层模块名列表
        :param target_dir: 待校验的插件内容目录
        :return: 缺失的模块名列表
        """
        import importlib
        import importlib.util
    ...
    def _write_pip_manifest(self, staged_dir: Path, name: str, python_exe: Path) -> None:
        """
        为 pip 安装结果补写 manifest.json，使其与 zip 安装的目录结构完全一致
        （_is_installed 依据 manifest 判定已安装）。
        :param staged_dir: pip 安装的暂存目录
        :param name: 插件标识
        :param python_exe: 执行安装的解释器（用于记录 Python 版本）
        """
        import json
        from datetime import datetime

        pins = PLUGIN_PIP_REQUIREMENTS[name]
        version = "pip:" + "+".join(pins)
        if len(version) > 200:
            version = version[:200] + "..."
        manifest = {
            "name": name,
            "display_name": PLUGIN_DEFINITIONS[name].display_name,
            "version": version,
            "modules": list(PLUGIN_DEFINITIONS[name].modules),
            "python_version": self._runtime_python_version(python_exe),
            "app_version": APP_VERSION,
            "built_at": datetime.now().isoformat(timespec="seconds"),
            "install_mode": "pip",
        }
        (staged_dir / MANIFEST_FILE).write_text(
            json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8"
        )

    def install_from_pip(self, name: str) -> tuple[bool, str]:
        """
        通过 pip 从包索引源安装插件依赖。

        执行方式：<内置 Python> -m pip install --target <暂存目录> <锁版本清单>，
        成功后校验插件要求的模块可定位、补写 manifest 并原子替换插件目录。
        依赖版本由 plugins/pip_pins.py 锁定（与插件 zip 构建时同一组版本），
        避免装到未经测试的最新版本。
        该方法为阻塞操作，调用方应放到后台线程执行。
        :param name: 插件标识
        :return: (是否成功, 结果消息)
        ...
        pins = PLUGIN_PIP_REQUIREMENTS.get(name)
        if not pins:
            return False, f"插件 {name} 没有可用的 pip 锁版本清单，请改用在线下载或本地安装"

        python_exe = self._locate_pip_python()
        if python_exe is None:
            return False, (
                "未找到可执行 pip 的 Python 运行时（内置运行时 runtime/python 不存在，"
                "当前解释器也无 pip），无法使用 Pip 安装；请先运行 "
                "scripts/setup_pip_runtime.py 准备内置运行时，或使用「在线下载」/「本地安装」。"
            )

        index_url = self.read_pip_index_url()
        staged_dir = Path(tempfile.mkdtemp(prefix=f"qtr-pip-{name}-"))
        command = [
            str(python_exe),
            "-m",
            "pip",
        ...
        try:
            logger.info(
                f"开始 pip 安装插件 name={name}, python={python_exe}, index={index_url}"
            )
            flags = subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0
            ...
            if completed.returncode != 0:
                ...
                logger.error(f"pip 安装失败 name={name}: {tail}")
                return False, f"pip 安装失败（退出码 {completed.returncode}）：{tail}"

            missing = self._check_modules_in_dir(definition.modules, staged_dir)
            if missing:
                logger.error(f"pip 安装缺少模块 name={name}: {missing}")
                return False, (
                    f"pip 安装完成但缺少插件要求的模块: {', '.join(missing)}，已放弃安装"
                )

            self._write_pip_manifest(staged_dir, name, python_exe)
            self._replace_plugin_dir(name, staged_dir)
            logger.info(f"插件 pip 安装成功 name={name}, index={index_url}")
            return True, (
                f"{definition.display_name} 插件通过 Pip 安装成功（源: {index_url}），"
                "重启客户端后生效"
            )
        except subprocess.TimeoutExpired:
            logger.error(f"pip 安装超时 name={name}")
            return False, "pip 安装超时（15 分钟），请检查网络或 Pip 源配置后重试"
        except Exception as e:
            logger.exception(f"pip 安装异常 name={name}: {e}")
            return False, f"pip 安装失败: {e}"
        finally:
            import shutil
