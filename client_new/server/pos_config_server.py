...
class PosConfigServer:
    ...
    @classmethod
    def logout_pos_account(cls, pos_path) -> None:
        ...
        if not pos_config or not isinstance(pos_config, PosParamsModel):
            raise PosHandleException(
                f"获取POS缓存失败， 无法注销POS账号: pos_config={pos_config}"
            )
        pos_env = PosConfig.get_local_pos_env(pos_path)
        if not pos_env or pos_env.upper() not in (
            "KH_TEST",
            "KH_TEST_S",
            "RTA_TEST",
            "RTA_UAT",
        ...
        status, message_info = pos_network.pos_account_logout(logout_model)
        if not status:
