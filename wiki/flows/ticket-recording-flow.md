...
录制事件可以作为门店配置操作模板的来源：录制完成后由实施人员整理变量、`fileKey`、断言和证据点，再由配置任务阶段引用模板版本。配置任务的阶段、输入资源、审批、运行记录和截图产物独立保存，不直接写入普通 Web 用例运行记录。

参见：[配置任务复用 Web 录制与执行](configuration-task-web-reuse.md)、[门店配置任务域设计](../entities/services/configuration-task-domain.md)。
