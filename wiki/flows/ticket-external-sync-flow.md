...
| 11.2 | 2026-09-07 起，`groupPush.aiResultFollowUp` 支持把 AI 分析结果回帖到工单群话题：AI 终态后先按真实场景补发工单信息群消息（未发送过则发送并记录锚点），再把分析结果回帖到 `group_push_message_refs` 锚点对应话题（按 chatId 去重，工单信息发到几个群就回几个群）；`sendOn` 支持 none（默认不推送）/success/failed/always，取消态永不回帖；按 AI task_id 幂等（`sync_state.ai_result_reply_task_ids` 保留最近 50 个）；无锚点时按 `noAnchorStrategy` 处理：skip（默认）记日志跳过防止对历史工单重复发送，send_then_reply 经 `send_group_message_for_ai_reply`（复用自动群推送全套判定，已发送过不重复）补发工单信息建立话题后回帖；手动提交分析可传 `aiResultFollowUp` 三态（follow 跟随全局/on/off）覆盖本次行为。 |
