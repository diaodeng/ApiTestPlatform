...
| 11 | 自动群推送采用”仅一次成功发送”标记：`group_push_sent_once=true` 后，即使后续是同工单更新也不会重复自动发群消息；手动发群不受此标记限制。2026-09-07 起，入库时会把本次同步场景持久化到 `extra_data.external_sync.sync_state.sync_scene`；AI 任务终态后的群推送回调（`finalize_sync_after_ai`）读取该真实场景判断场景开关（如多维表格拉取按 `sendAfterBitablePull`），历史工单无该字段时按来源系统推断（`feishu_bitable_pull`→bitable_pull、`manual_create`→manual_create、其余→external_sync），不再硬编码 external_sync；AI 终态同时把自动化步骤 `ai_analysis` 从 queued 收敛为 success/failed/canceled。 |
...
| 11.2 | 2026-09-07 起，`groupPush.aiResultFollowUp` 支持把 AI 分析结果回帖到工单群话题：AI 终态后先按真实场景补发工单信息群消息（未发送过则发送并记录锚点），再把分析结果回帖到锚点表对应话题（按 chatId 去重，工单信息发到几个群就回几个群）；`sendOn` 支持 none（默认不推送）/success/failed/always，取消态永不回帖；幂等判定走 `ticket_ai_analysis_task.result_replied_at` 列——2026-09-11 阶段二起为**抢占式**（发送前条件 UPDATE 占坑并提交，占坑失败即并发已处理、全部群发送失败自动释放占坑允许重试），粒度由 `oncePerTicket` 控制（false 默认任务级：一任务一回；true 工单级：工单回帖成功过一次即不再回，手动 override=on 跳过）；锚点走独立表 `ticket_group_push_anchor`；无锚点时按 `noAnchorStrategy` 处理：skip（默认）记日志跳过防止对历史工单重复发送，send_then_reply 经 `send_group_message_for_ai_reply`（复用自动群推送全套判定，已发送过不重复）补发工单信息建立话题后回帖；手动提交分析可传 `aiResultFollowUp` 三态（follow 跟随全局/on/off）覆盖本次行为；手动补发接口 `POST /ticket/{ticket_id}/ai-analysis/tasks/{task_id}/reply-resend` 对未回帖的终态任务直接读取持久化结果补发（强制 enabled+sendOn=always+override=on，拒绝已回帖任务，结果写工单事件）。迁移历史：2026-09-10 前锚点/幂等存 `sync_state` JSON 被白名单重建静默擦除（生产 INC00001934853/R 定位根因），09-10 白名单补齐止血，09-11 拆表根治（DDL `sql/20260911_ticket_group_push_anchor.sql` + 回填 `scripts/migrate_group_push_anchor.py`）。消息形态：2026-09-11 起回帖模板留空（内置默认）时按 `aiResultFollowUp.messageStyle` 决定（card 默认 / text），card 发**飞书交互卡片**（`TicketAiResultReplyCardService` 构造，成功绿头、失败红头），text 发默认纯文本模板；配置了自定义 `template` 的始终走纯文本。卡片区块由 `aiResultFollowUp.cardFields` 白名单控制（`CARD_FIELD_KEYS` 九项：ticket_info/analysis_summary/root_cause/fix_suggestion/evidence/risk_items/next_steps/confidence/ticket_link，留空全量、无效回退全量、字段配置但载荷为空不渲染、失败原因区块保留）。`send_feishu_thread_reply` 新增可选 `card` 参数按 `msg_type=interactive` 回帖，不传保持纯文本。 |
...
| 13 | 群推送应用发送成功后会把飞书 `messageId/rootId/threadId/chatId` 写入独立表 `ticket_group_push_anchor`（2026-09-11 拆表；此前存 `extra_data.external_sync.sync_state.group_push_message_refs`，被 build_meta 白名单重建静默擦除导致回帖丢失），单工单保留最近 20 条，`message_id` 唯一约束去重；后续飞书事件优先按这些锚点索引匹配工单（`TicketGroupPushAnchorDao.match_ticket_ids_by_message_values`，替代全表扫描），历史无锚点消息回退从文本识别工单号。 |
