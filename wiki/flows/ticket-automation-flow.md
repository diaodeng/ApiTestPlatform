...
| Agent 执行中连接中断 | 任务进入 `pending_recovery` 恢复等待（非失败，不发失败通知），Agent 重连补交后由恢复扫描任务自动写回成功；超恢复期限未补交才置败并通知。 |
