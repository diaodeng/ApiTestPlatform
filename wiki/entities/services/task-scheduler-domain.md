...
- 非 Windows 环境下，业务任务仍通过 `spawn` 子进程执行，结果回传改为 `Pipe` 直接通信，避免队列 flush 竞态导致父进程误判“子进程未返回结果”。
- Windows 开发环境下，由于 `spawn` 子进程启动时容易丢失项目模块导入路径，任务会退化为在 Celery Worker 线程内直接执行，优先保证本地开发可用性。
