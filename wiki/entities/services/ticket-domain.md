...
- 2026-07-17 起，工单管理页内的详情全屏弹窗由 `TicketDetailWithList.vue` 自闭环承接，父页只传 `ticketId/open` 并监听 `changed/closed`；详情组件内部自行加载详情、描述翻译、评论、时间线、协同消息、RCA、日志拉取、AI 任务、仓库映射、商家映射、问题绑定和日志查看器。下方 tabs 已拆为概览、日志拉取、协同、评论、历史 5 个子组件，父页不再持有 `ticketDetailContext` 或详情弹窗状态。
