...
related_files:
  ...
  - server/modules/ticket/service/sync/ticket_sync_ai_config_service.py
...
- 2026-07-27 起，`TicketSyncAiConfigService` 统一解析 `translateConfig`、`titleSummaryConfig`、`aiClassification`、`aiSyncExtract` 和 `knowledgeConfig`。旧 `ticket.ai.translate.*`、`ticket.ai.title.summary.*`、`ticket.ai.category.classify.*`、`ticket.ai.log_extract.*`、`ticket.ai.knowledge.*` 即使仍存在数据库中也已失效，代码不读取、不写入且不做数据迁移或兼容回退。
- 多维表格主动拉取的自动化优先级为“定时任务显式 `automation` > `ticket.sync.automation` 场景配置”；任务参数未提供 `automation` 时保持 `None`，不能用模型默认值遮蔽全局场景配置。全局 `bitablePull.automation` 保存时会被移除。
...
- 2026-07-04 起，工单详情页和日志拉取管理页的日志拉取列表统一展示“拉取参数”，日志类型显示 `modifyTime`，数据库类型显示 `path`；归档地址和原始压缩包列改为链接文本，左键打开或下载，右键直接复制链接，不再依赖“下载/复制”双按钮组合。
...
- 日志拉取轮询已改为后台任务查询：提交申请与轮询探测解耦，`_poll_external_result` 阻塞轮询（原最长占线程 1800s）已删除。创建记录后线程池只做瞬时提交申请，提交成功写入 `poll_deadline_at`（= now + pollTimeoutSec）；轮询探测由 Celery 周期任务 `module_task.scheduler_maintenance.scan_log_pull_records` 驱动（与项目其他定时任务一致，需在「系统监控-定时任务」手动配置，建议 interval 30 秒，不在启动时自动注册）：created 兜底提交、submitting/polling 单次探测（命中可下载投递下载解析、外部失败标记失败）、超过 `poll_deadline_at` 标记「轮询外部平台超时」失败。任务提交与轮询探测不再受线程池并发数限制，仅下载/解析阶段复用线程池。
