...
- 2026-07-17 起，工单管理页的详情全屏弹窗由 `TicketDetailWithList.vue` 自闭环承接，列表页仅传 `ticketId/open` 并监听 `changed/closed`；详情组件内部管理详情数据、描述/AI 翻译、AI 分析、任务历史、仓库映射、问题归因和商家映射弹窗。下方内容区拆为概览、日志拉取、协同、评论、历史 5 个 tab 子组件，tab 只接收 `ticketId/active`，自行拉取数据、维护表单/弹窗/样式并提交变更。
