...
- AI Provider 可观测上报（2026-09-02）：Provider 支持配置 OTLP 上报端点与鉴权（`observability_*` 字段，密钥 Fernet 加密）；服务端在 `server/modules/ticket/service/ai/ticket_ai_observability_service.py` 只解析配置并经 providerEnv 下发，任务级 LLM span 由 Agent 侧 `client_new/services/ticket_ai_observability_service.py` 直连平台上报（`input.value`/`output.value`/`gen_ai.usage.*`，session=`ticket-ai-task-{taskId}`，旁路 best-effort），并按 `observability_cli_enabled` 向 client_new 下发 OTEL 环境注入本机 Codex（config.toml `[otel]` 托管段）与 Claude Code（环境变量 + `TRACEPARENT`）。
